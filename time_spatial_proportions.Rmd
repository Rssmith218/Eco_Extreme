...
spatial3 <- as.data.frame.matrix(spatial2, make.names = TRUE)
spatial3 <- setDT(spatial3, keep.rownames = TRUE)[]
spatial3$`undefined` <- 0
spatial3$`1-10 sq m` <- 0
spatial4 <- as.xtabs(spatial3)

spatial_df1 <- as.data.frame(spatial4)
...
*Sampling Unit Spatial Extent vs. Event Spatial Extent - as a proportion:*
  ```{r sample2,  fig.width = 12, fig.height = 12}
##convert xtabs to data.frame, add in missing categories, convert back to xtabs, covert xtabs to dataframe for heat maps##
sample2 <- as.data.frame.matrix(sample1, make.names = TRUE)
sample2 <- setDT(sample2, keep.rownames = TRUE)[]
sample2$`undefined` <- 0
sample2$`1-10 sq m` <- 0
sample3 <- as.xtabs(sample2)

sample_df1 <- as.data.frame(sample3)
